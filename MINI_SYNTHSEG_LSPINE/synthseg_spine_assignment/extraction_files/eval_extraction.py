...
# -------------------------------------------------------
# 설정: 경로 변수 (T2 평가 데이터 경로)
# -------------------------------------------------------
# T1/T2 파일이 모두 포함된 원본 MHA 데이터 폴더 (T2 파일도 여기에 있다고 가정)
INPUT_MHA_DIR = r"C:\Users\jooyoung\Desktop\Code\03_fine_tuning\MINI_SYNTHSEG_LSPINE\data"

# T2 이미지/마스크 파일이 있는 하위 폴더 이름
INPUT_T2_IMG_MHA_DIR = os.path.join(INPUT_MHA_DIR, "images") 
INPUT_T2_MASK_MHA_DIR = os.path.join(INPUT_MHA_DIR, "masks") 

# 출력 경로: T2 평가용 PNG 슬라이스가 저장될 폴더
OUTPUT_T2_IMG_DIR = r"C:\Users\jooyoung\Desktop\Code\03_fine_tuning\MINI_SYNTHSEG_LSPINE\eval_data\images" 
OUTPUT_T2_MASK_DIR = r"C:\Users\jooyoung\Desktop\Code\03_fine_tuning\MINI_SYNTHSEG_LSPINE\eval_data\masks"

os.makedirs(OUTPUT_T2_IMG_DIR, exist_ok=True)
os.makedirs(OUTPUT_T2_MASK_DIR, exist_ok=True)
...
# 메인 변환 로직 (T2 필터링 추가)
# -------------------------------------------------------
def convert_t2_mha_to_png_slices(img_mha_dir, mask_mha_dir, img_out_dir, mask_out_dir):
    """3D T2 MHA 파일에서 2D 슬라이스를 추출하여 PNG로 저장"""
    
    # 1. 파일 목록 가져오기 (T2 파일만 필터링)
    # T2 파일 이름 규칙: 'X_t2.mha' 형태를 가정합니다. 
    mha_files = sorted([f for f in os.listdir(mask_mha_dir) if f.endswith('.mha') and '_t2' in f.lower()])
    
    if len(mha_files) == 0:
        print("🚨 Error: No T2 MHA files found in the mask directory. Check the path and file naming.")
        return

    for mha_name in tqdm(mha_files, desc="Slicing Pure T2 MHA Files for Evaluation"):
        # 파일 이름을 ID로 사용 (예: 1_t2.mha -> 1_t2)
    ...
    print(f"\n✅ Pure T2 Evaluation Data Conversion Complete. PNG slices saved to:\n - {OUTPUT_T2_IMG_DIR}\n - {OUTPUT_T2_MASK_DIR}")

# 변환 함수 실행
convert_t2_mha_to_png_slices(INPUT_T2_IMG_MHA_DIR, INPUT_T2_MASK_MHA_DIR, OUTPUT_T2_IMG_DIR, OUTPUT_T2_MASK_DIR)